...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--repo",
        help="Slug owner/repo para consultar issues publicas de GitHub. Si no se indica, se intenta inferir de origin.",
    )
...
def summarize_contributors(commits: Iterable[Commit]) -> list[ContributorSummary]:
    ...
    for commit in commits:
        ...
        if email not in grouped:
            grouped[email] = ContributorSummary(
    ...
    return sorted(grouped.values(), key=lambda item: (-item.commits, item.display_name.lower()))


def extract_rf_numbers(paths: Iterable[Path]) -> set[int]:
    found: set[int] = set()
    for path in paths:
        match = FILE_RF_RE.search(path.name)
        if match:
            found.add(int(match.group(1)))
    return found


def collect_local_artifacts() -> dict[str, set[int]]:
    requirements = list(REQUIREMENTS_DIR.glob("*.md"))
    modeling = list(MODELING_DIR.glob("rf*.*"))
    design = list(DESIGN_DIR.glob("*.*"))
    prototypes = list(PROTOTYPES_DIR.glob("rf*.*"))
    return {
...
def fetch_all_issues(repo_slug: str) -> list[dict]:
    ...
    page = 1
    while True:
        url = f"https://api.github.com/repos/{repo_slug}/issues?state=all&per_page=100&page={page}"
        request = Request(
            url,
            headers={
...
def contributor_work_percentage(item: ContributorSummary, contributors: list[ContributorSummary]) -> str:
    total_changed_lines = sum(contributor.changed_lines for contributor in contributors)
    if total_changed_lines > 0:
        return percent(item.changed_lines, total_changed_lines)

    total_commits = sum(contributor.commits for contributor in contributors)
    return percent(item.commits, total_commits)
...
def render_report(
    ...
    open_issues = [issue for issue in issues if issue.get("state", "").lower() == "open"]
    observations = project_observations(contributors, rf_matrix, issues, issue_error)

    contributor_rows = []
    for item in contributors:
        highlights = "; ".join(item.commit_messages[:3]) if item.commit_messages else "Sin commits en el rango"
        contributor_rows.append(
            [
                item.display_name,
                contributor_work_percentage(item, contributors),
                str(item.commits),
                str(len(item.files_touched)),
                str(item.insertions),
                str(item.deletions),
                highlights,
            ]
    ...
    if not task_rows:
        non_merge_commits = [commit for commit in commits if not commit.subject.lower().startswith("merge pull request")]
    ...
    progress_rows = [
        ...
        ["Issues cerradas", str(progress["closed_issues"]), percent(int(progress["closed_issues"]), int(progress["total_issues"]))],
        ["Issues abiertas", str(progress["open_issues"]), percent(int(progress["open_issues"]), int(progress["total_issues"]))],
        ["RF cerrados", f"{progress['rf_closed']}/{progress['rf_total']}", percent(int(progress["rf_closed"]), int(progress["rf_total"]))],
        ["RNF cerrados", f"{progress['rnf_closed']}/{progress['rnf_total']}", percent(int(progress["rnf_closed"]), int(progress["rnf_total"]))],
        ["RF con documento", f"{progress['rf_with_requirements']}/{progress['rf_total']}", percent(int(progress["rf_with_requirements"]), int(progress["rf_total"]))],
        ["RF con diagrama", f"{progress['rf_with_modeling']}/{progress['rf_total']}", percent(int(progress["rf_with_modeling"]), int(progress["rf_total"]))],
        ["RF con diseno", f"{progress['rf_with_design']}/{progress['rf_total']}", percent(int(progress["rf_with_design"]), int(progress["rf_total"]))],
    ]
    ...
    lines = [
        f"# Bitacora semanal del proyecto",
        "",
        ...
        render_table(
            ...
            contributor_rows or [["Sin actividad", "0%", "0", "0", "0", "0", "No hubo commits en el rango"]],
        ),
        "",
